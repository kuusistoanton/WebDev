...
navigator.geolocation.getCurrentPosition((position) => {
  ...
  restaurants.sort((a, b) => {
    const distanceA = Math.sqrt(
      Math.pow(a.location.coordinates[1] - userLatitude, 2) +
        Math.pow(a.location.coordinates[0] - userLongitude, 2)
    );
    const distanceB = Math.sqrt(
      Math.pow(b.location.coordinates[1] - userLatitude, 2) +
        Math.pow(b.location.coordinates[0] - userLongitude, 2)
    );
  ...
  restaurants.forEach((restaurant) => {
    ...
    listItem.textContent = restaurant.name + ' - ' + restaurant.address + ', ' + restaurant.city;
