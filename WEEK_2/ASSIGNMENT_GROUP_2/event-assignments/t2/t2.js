...
document.addEventListener('DOMContentLoaded', () => {
  // inject styles for list, highlight and modal
  const style = document.createElement('style');
  style.textContent = `
    .restaurant-list { list-style:none; padding:0; margin:16px auto; max-width:900px; font-family:Arial, sans-serif; }
    .restaurant-item { padding:12px 16px; border-bottom:1px solid #e6e6e6; cursor:pointer; display:block; background:#fff; transition:background .15s; }
    .restaurant-item:hover { background:#f9f9f9; }
    .restaurant-item .name { display:block; font-size:1.05rem; color:#222; }
    .restaurant-item .address { display:block; font-size:0.95rem; color:#555; margin-top:4px; }
    .highlight { background:#fff7cc !important; font-weight:700; }
    .modal-overlay { position:fixed; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.45); z-index:10000; }
    .modal { background:#ffffff; padding:20px; border-radius:8px; box-shadow:0 8px 30px rgba(0,0,0,0.2); max-width:520px; width:90%; font-family:Arial, sans-serif; }
    .modal h2 { margin:0 0 8px 0; font-size:1.2rem; }
    .modal p { margin:6px 0; color:#333; }
    .modal .meta { color:#555; font-size:0.95rem; }
    .modal .close-btn { margin-top:12px; padding:8px 12px; cursor:pointer; border:1px solid #ccc; background:#f5f5f5; border-radius:4px; }
  `;
  ...
  // create and insert list container
  ...
  // create a modal showing restaurant details
  function openDetailsModal(r) {
    ...
    modal.innerHTML = `
      ...
      <p class="meta"><strong>Address:</strong> ${escapeHtml(r.address)}</p>
      <p class="meta"><strong>Postal code:</strong> ${escapeHtml(r.postalCode)}</p>
      <p class="meta"><strong>City:</strong> ${escapeHtml(r.city)}</p>
      <p class="meta"><strong>Phone:</strong> ${escapeHtml(r.phone)}</p>
      <p class="meta"><strong>Company:</strong> ${escapeHtml(r.company)}</p>
    `;
  ...
  // render list items with highlight behavior and click -> modal
